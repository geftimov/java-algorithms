package com.eftimoff.algorithms.support;
...
/**
 * <i>Standard draw</i>. This class provides a basic capability for creating drawings with your programs. It uses a simple graphics
 * model that allows you to create drawings consisting of points, lines, and curves in a window on your computer and to save the
 * drawings to a file.
 * <p>
 * For additional documentation, see <a href="http://introcs.cs.princeton.edu/15inout">Section 1.5</a> of <i>Introduction to
...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener
{
    ...
    /**
     * Set the pen color to the given color. The available pen colors are BLACK, BLUE, CYAN, DARK_GRAY, GRAY, GREEN, LIGHT_GRAY,
    ...
    /**
     * Display on screen, pause for t milliseconds, and turn on <em>animation mode</em>: subsequent calls to drawing methods such as
     * <tt>line()</tt>, <tt>circle()</tt>, and <tt>square()</tt> will not be displayed on screen until the next call to
     * <tt>show()</tt>. This is useful for producing animations (clear the screen, draw a bunch of shapes, display on screen for a
     * fixed amount of time, and repeat). It also speeds up drawing a huge number of shapes (call <tt>show(0)</tt> to defer drawing
     * on screen, draw the shapes, and call <tt>show(0)</tt> to display them all on screen at once).
     * 
    ...
    /**
     * Display on-screen and turn off animation mode: subsequent calls to drawing methods such as <tt>line()</tt>, <tt>circle()</tt>
    ...
    /**
     * What is the next key that was typed by the user? This method returns a Unicode character corresponding to the key typed (such
     * as 'a' or 'A'). It cannot identify action keys (such as F1 and arrow keys) or modifier keys (such as control).
     * 
    ...
    /**
     * Is the keycode currently being pressed? This method takes as an argument the keycode (corresponding to a physical key). It
     * can handle action keys (such as F1 and arrow keys) and modifier keys (such as shift and control). See <a href =
     * "http://download.oracle.com/javase/6/docs/api/java/awt/event/KeyEvent.html">KeyEvent.java</a> for a description of key codes.
     * 
